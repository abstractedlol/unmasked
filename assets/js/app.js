    ...
    function getSession() {
      return window.UnmaskedSupabase.getSession();
    }
    ...
    function hasDashboardAccess() {
      return !!getSession() && workspaceAccess;
    }
    ...
    function updateAuthUI() {
      ...
      if (session) {
        const name = window.UnmaskedSupabase.displayName(session);
    ...
    function resultType(tool) {
      const folder = RESULT_FOLDERS[tool];
      if (!folder) throw new Error(`${tool} is not connected to a data source yet.`);
      return folder;
    }

    async function fetchQueryResults(tool, query) {
      return window.UnmaskedSupabase.lookup(resultType(tool), query);
    }
    ...
    async function executeLookup(query) {
      ...
      const steps = [
        'Connecting to the secured data service…',
        'Checking workspace permissions…',
        'Querying indexed records…',
      ...
      try {
        ...
        const payload = await withMinimumDelay(fetchQueryResults(targetTool, cleaned), responseDelay);
        ...
        if (activeTool === stateKey) {
          ...
          resultsGrid.innerHTML = `<div class="result-empty">${escapeHtml(error.message)}</div>`;
          resultsPagination.hidden = true;
          resultsContainer.hidden = false;
        }
      } finally {
        ...
        toolInput.disabled = !hasSearchAccess();
    ...
    document.getElementById('authForm').addEventListener('submit', async e => {
      ...
      try {
        if (mode === 'signup') {
          const result = await window.UnmaskedSupabase.signUp(email, password);
          authPassword.value = '';
          if (result.session) {
            workspaceAccess = await window.UnmaskedSupabase.hasWorkspaceAccess();
            updateAuthUI();
            closeModal(signin);
          } else {
            setAuthMode('login');
            showAuthMessage('Account created. Check your email, then log in.', 'success');
            authPassword.focus();
          }
        } else {
          await window.UnmaskedSupabase.signIn(email, password);
          workspaceAccess = await window.UnmaskedSupabase.hasWorkspaceAccess();
          authPassword.value = '';
          updateAuthUI();
          closeModal(signin);
        ...
        showAuthMessage(error.message || 'Could not connect to account services.', 'error');
      } finally {
    ...
    async function logoutAccount() {
      await window.UnmaskedSupabase.signOut();
    ...
    async function hydrateSessionAccess() {
      try {
        const session = await window.UnmaskedSupabase.restoreSession();
        workspaceAccess = session ? await window.UnmaskedSupabase.hasWorkspaceAccess() : false;
        updateAuthUI();
      } catch {
        workspaceAccess = false;
        updateAuthUI();
